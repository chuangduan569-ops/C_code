...
static char *get_file_mtime(time_t tm, char *tbuf)
{
	...
	strftime(tbuf, TIMELEN, "%m月 %d %H:%M", p);	//把时间结构转换成格式化结构的字符串
	return tbuf;	//返回转换完的格式化时间字符串
}
static	char *get_file_name(const char *pathname)
{
	return (char *)pathname;	//直接返回文件名
}
...
int main(int argc, char *argv[])
{
	...
	printf("%ld ",get_file_size(fs.st_size));//获取文件的大小
	printf("%s ", get_file_mtime(fs.st_mtime, tbuf));//获取文件时间
	printf("%s\n", get_file_name(argv[1]));	//获取文件名
