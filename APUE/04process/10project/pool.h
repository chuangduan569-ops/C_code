...
#define MAXJOB	10 //最大任务数量
#define MIN_FREE_NR	3 //最小空闲的个数
#define MAX_FREE_NR 5 //最大空闲的个数
#define STEP	2 //增量(增加或减少线程的数量)

//[1] 定义线程池结构体
typedef struct 
{
	pthread_t *workers;//工作线程结构体的起始地址
	...
	int exit_threads;//管理者线程当需要终止线程的时候 进行记录
	int shutdown;//标记是否关闭线程池 (0正常工作 1线程池关闭)
	pthread_mutex_t mut_pool;//整个线程池的互斥量  (同步  所以需要加锁)
	pthread_mutex_t mut_busy;//busy单独互加一个斥量
	pthread_cond_t queue_not_empyt;//如果队列不为空 通知取任务
