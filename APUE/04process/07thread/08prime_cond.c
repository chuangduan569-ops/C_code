...
*/
	...
	while(1)
	{
...
#if defined(COND)
		while(job == 0)//因为会出现死锁的问题 所以改为while
		{
		...
		job = 0;//修改取走后临界区的状态
#if defined(COND)
		pthread_cond_broadcast(&cond);//由于改变了job值 发送条件变量变化的通知
#endif
		pthread_mutex_unlock(&mutex);//解锁
...
int main(void)
{
...
#if defined(COND)
		/*
		[1]unlock [2]等待条件变量变化的通知 [3]lock  原子操作 
		*/
		pthread_cond_wait(&cond, &mutex);//等待条件变量变化的通知
#else
		pthread_mutex_unlock(&mutex);//解锁
	...
	job = i;//main()线程发放新的任务
#if defined(COND)
	pthread_cond_signal(&cond);//发送条件变量变化的通知(只对一个线程即可)
#endif
	pthread_mutex_unlock(&mutex);//解锁
	}
