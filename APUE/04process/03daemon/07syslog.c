...
{   
    ...
    {
        //perror("fork()");//打印错误信息
        syslog(LOG_ERR, "fork() : %s", strerror(errno));//生成一条日志信息
...
//        perror("setsid()");//打印错误信息
  		syslog(LOG_ERR, "setsid() : %s", strerror(errno));//生成一条日志信息
    ...
    {
        //perror("chdir()");//打印错误信息
        syslog(LOG_ERR, "chdir() : %s", strerror(errno));//生成一条日志信息
    ...
    {
        //perror("open()");//打印错误信息
        syslog(LOG_ERR, "open() : %s", strerror(errno));//生成一条日志信息
...
static int process_already_running(void)//获取文件互斥锁
{
    ...
    {
      //  perror("flock()");//打印错误信息
       	syslog(LOG_ERR, "flock() : %s", strerror(errno));//生成一条日志信息
		close(fd);//关闭锁文件
    ...
    if(ftruncate(fd, 0) == -1)//判断把锁文件截断为0是否失败
    {
       // perror("ftruncate()");//打印错误信息
       	syslog(LOG_ERR, "ftruncate() : %s", strerror(errno));//生成一条日志信息
		close(fd);//关闭锁文件
...
int main(int argc, char *argv[])
{
	...
	//与系统日志文件建立链接(显示PID 和 STDERR(标准出错)) 来源(守护进程)
...
//        fprintf(stderr, "process_already_running() Is Failed!\n");//打印错误信息
    ...
    {
       // perror("fopen()");//打印错误信息
        syslog(LOG_ERR, "fopen() : %s", strerror(errno));//生成一条日志信息
		ret = -1;//设置错误码为-1
        goto ERR_1;//跳转到ERR_1的标志
    }
    ...
    while(1)//死循环
    {
        ...
        {
           // perror("time()");//打印错误信息
            syslog(LOG_ERR, "time() : %s", strerror(errno));//生成一条日志信息
			ret = -2;//设置错误码为-2
       ...
       //     perror("localtime()");//打印错误信息
         	syslog(LOG_ERR, "localtime() : %s", strerror(errno));//生成一条日志信息
			ret = -3;//设置错误码为-3
            goto ERR_2;//跳转到ERR_2的标志
        }
        //memset();//清空脏数据
