...
int main(int argc, char *argv[])
{
    ...
    if(argc != 5)  // 原来是 < 3
    {
        ...
        fprintf(stderr, "Example: %s 192.168.1.100 9527 1 Hello\n", argv[0]);
        return -1;
    }
/*	if(argc < 3)//判断命令行参数的个数是否少于3个
	...
	//[2] 发送数据报
	buf.id = atoi(argv[3]);//把argv[1]转换为整形的ID
	strncpy(buf.msg, argv[4], MSGSIZE);//把argv[2]拷贝到msg
	sever_addr.sin_family = AF_INET;//指定IPv4协议

	inet_aton(argv[1], &sever_addr.sin_addr);  // 新增这一行，原来用的是 SERVER_IP
