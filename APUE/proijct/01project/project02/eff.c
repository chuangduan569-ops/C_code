...
// 文件比较函数（按名称排序）
static int compare_ls_files(const void *a, const void *b)
{
    const LsFileInfo *fa = (const LsFileInfo*)a;    // 将a转换为文件信息结构体指针
    const LsFileInfo *fb = (const LsFileInfo*)b;    // 将b转换为文件信息结构体指针
    return strcmp(fa->name, fb->name);              // 按文件名比较返回结果
}

// 打印文件大小（支持 -h 选项）
static void print_ls_size_hr(off_t size, int human_readable)
{
    if(!human_readable)                              // 如果没有-h选项
    {
        printf("%8ld ", (long)size);                 // 直接打印字节数
        return;                                      // 返回
    }
    
    const char *units[] = {"B", "K", "M", "G", "T"}; // 容量单位数组
    int unit = 0;                                    // 当前单位索引初始0
    double s = size;                                 // 大小转换为双精度
    
    while(s >= 1024 && unit < 4)                     // 当大于1024且未到最大单位
    {
        s /= 1024;                                   // 除以1024
        unit++;                                      // 单位索引加1
    }
    ...
    if(unit == 0)                                    // 如果是字节单位
        printf("%4.0f%s ", s, units[unit]);          // 打印整数
    else                                             // 其他单位
        printf("%4.1f%s ", s, units[unit]);          // 打印一位小数
}

// 获取用户名（复用现有函数）
static char *get_ls_uname(uid_t st_uid)
{
    struct passwd *p = getpwuid(st_uid);             // 根据uid获取用户信息
    if(p == NULL)                                    // 如果获取失败
        return "unknown";                            // 返回unknown
    return p->pw_name;                               // 返回用户名
}

// 获取组名（复用现有函数）
static char *get_ls_gname(gid_t st_gid)
{
    struct group *p = getgrgid(st_gid);              // 根据gid获取组信息
    if(p == NULL)                                    // 如果获取失败
        return "unknown";                            // 返回unknown
    return p->gr_name;                               // 返回组名
}

// 获取修改时间（复用现有函数）
static char *get_ls_mtime(time_t tm, char *tbuf, size_t len)
{
    struct tm *p = localtime(&tm);                   // 将时间戳转换为本地时间结构
    if(p == NULL)                                    // 如果转换失败
    {
        strncpy(tbuf, "unknown", len);               // 复制unknown到缓冲区
        return tbuf;                                 // 返回unknown
    }
    strftime(tbuf, len, "%m月 %d %H:%M", p);         // 格式化时间字符串：月 日 时:分
    return tbuf;                                     // 返回时间字符串
}
...
{
    optind = 1;                                      // 重置选项索引
    opterr = 0;                                      // 关闭getopt的错误输出

    struct stat fs;                                  // 文件状态结构体
    LsOptions opts = {0};                            // 初始化选项为0
    char *path = ".";                                // 默认路径为当前目录
    char buf[12] = {0};                              // 权限字符串缓冲区
    char tbuf[32] = {0};                             // 时间字符串缓冲区
    int opt;                                         // 选项变量
    int ret = 0;                                     // 返回值
    
    // 解析选项
    while((opt = getopt(argc, argv, "alihd")) != -1) // 循环解析选项
    {
        switch(opt)                                  // 根据选项判断
        {
            case 'a': opts.show_all = 1; break;      // -a: 显示所有文件
            case 'l': opts.long_format = 1; break;   // -l: 长格式显示
            case 'i': opts.show_inode = 1; break;    // -i: 显示inode号
            case 'h': opts.human_readable = 1; break;// -h: 人性化显示
            case 'd': opts.show_directory = 1; break;// -d: 显示目录本身
        }                                            // switch结束
    }                                                // 循环结束
    
    if(optind < argc)                                // 如果有路径参数
    {
        path = argv[optind];                         // 获取第一个路径参数
    }                                                // 条件结束
    
    if(stat(path, &fs) == -1)                        // 获取文件状态失败
    {
        perror("stat()");                            // 打印错误信息
        return -1;                                   // 返回-1
    }                                                // 条件结束
    ...
    // 处理文件或 -d 选项
    if(opts.show_directory || !S_ISDIR(fs.st_mode))  // 如果是-d或普通文件
    {
        if(opts.show_inode)                          // 如果有-i选项
        {
            printf("%8ld ", (long)fs.st_ino);        // 打印inode号
        }                                            // 条件结束
        
        if(opts.long_format)                         // 如果有-l选项
        {
            printf("%c", get_file_type(fs.st_mode)); // 打印文件类型
            printf("%s", get_file_permission(fs.st_mode, buf)); // 打印权限
            printf(" %d ", (int)fs.st_nlink);        // 打印硬链接数
            printf("%s", get_ls_uname(fs.st_uid));   // 打印用户名
            printf(" %s ", get_ls_gname(fs.st_gid)); // 打印组名
            print_ls_size_hr(fs.st_size, opts.human_readable); // 打印文件大小
            printf("%s", get_ls_mtime(fs.st_mtime, tbuf, sizeof(tbuf))); // 打印修改时间
            printf(" %s\n", path);                   // 打印文件名换行
        }                                            // 条件结束
        else                                         // 没有-l选项
        {
    ...
    // 处理目录
    glob_t gs;                                       // glob结果结构体
    char pattern[1024];                              // 匹配模式缓冲区
    
    memset(&gs, 0, sizeof(gs));                      // 初始化glob结构体
    
    snprintf(pattern, sizeof(pattern), "%s/*", path); // 构造匹配普通文件的模式
    errno = 0;                                       // 清空errno
    int glob_ret = glob(pattern, GLOB_NOSORT, NULL, &gs); // 执行glob匹配
    
    if(glob_ret != 0 && glob_ret != GLOB_NOMATCH)    // 如果出错且不是无匹配
    {
        perror("glob()");                            // 打印错误信息
        return -2;                                   // 返回-2
    }                                                // 条件结束
    
    if(opts.show_all)                                // 如果有-a选项
    {
        char hidden_pattern[1024];                   // 隐藏文件模式缓冲区
        snprintf(hidden_pattern, sizeof(hidden_pattern), "%s/.*", path); // 构造匹配隐藏文件的模式
        errno = 0;                                   // 清空errno
        glob_ret = glob(hidden_pattern, GLOB_APPEND | GLOB_NOSORT, NULL, &gs); // 追加匹配隐藏文件
        
        if(glob_ret != 0 && glob_ret != GLOB_NOMATCH) // 如果出错且不是无匹配
        {
            perror("glob()");                        // 打印错误信息
            globfree(&gs);                           // 释放glob内存
            return -3;                               // 返回-3
        }                                            // 条件结束
    }                                                // 条件结束
    
    if(gs.gl_pathc == 0)                             // 如果没有匹配到文件
    {
        globfree(&gs);                               // 释放glob内存
        return 0;                                    // 返回0（空目录不是错误）
    }                                                // 条件结束
    
    // 创建文件信息数组
    LsFileInfo *files = malloc(gs.gl_pathc * sizeof(LsFileInfo)); // 分配内存
    if(files == NULL)                                // 分配失败
    {
    ...
    int file_count = 0;                              // 文件计数初始0
    
    for(size_t i = 0; i < gs.gl_pathc; i++)          // 遍历所有匹配的文件
    {
        // 获取文件名（去掉路径部分）
        char *filename = strrchr(gs.gl_pathv[i], '/'); // 查找最后一个斜杠
        if(filename)                                 // 如果找到了斜杠
            filename++;                              // 跳过斜杠
        else                                         // 如果没有斜杠
            filename = gs.gl_pathv[i];               // 直接使用路径
        ...
        struct stat st;                              // 文件状态结构体
        if(lstat(gs.gl_pathv[i], &st) == -1)         // 获取状态失败
            continue;                                // 跳过这个文件
        
        strncpy(files[file_count].name, filename, sizeof(files[file_count].name) - 1); // 复制文件名
        files[file_count].name[sizeof(files[file_count].name) - 1] = '\0'; // 确保字符串结束
        files[file_count].inode = st.st_ino;         // 保存inode号
        files[file_count].st = st;                   // 保存文件状态
        file_count++;                                // 文件计数加1
    }                                                // 循环结束
    
    globfree(&gs);                                   // 释放glob内存
    
    if(file_count == 0)                              // 如果没有有效文件
    {
        free(files);                                 // 释放内存
        return 0;                                    // 返回0
    }                                                // 条件结束
    
    qsort(files, file_count, sizeof(LsFileInfo), compare_ls_files); // 按文件名排序
    
    if(opts.long_format)                             // 如果有-l选项
    {
        long total_blocks = 0;                       // 总块数初始0
        for(int i = 0; i < file_count; i++)          // 循环累加
        {
            total_blocks += files[i].st.st_blocks;   // 累加每个文件的块数
        }                                            // 循环结束
        printf("总计 %ld\n", total_blocks / 2);      // 打印总块数
    }                                                // 条件结束
    
    for(int i = 0; i < file_count; i++)              // 循环显示每个文件
    {
        if(opts.show_inode)                          // 如果有-i选项
        {
            printf("%8ld ", (long)files[i].st.st_ino); // 打印inode号
        }                                            // 条件结束
        
        if(opts.long_format)                         // 如果有-l选项
        {
            printf("%c", get_file_type(files[i].st.st_mode)); // 打印文件类型
            printf("%s", get_file_permission(files[i].st.st_mode, buf)); // 打印权限
            printf(" %d ", (int)files[i].st.st_nlink); // 打印硬链接数
            printf("%s", get_ls_uname(files[i].st.st_uid)); // 打印用户名
            printf(" %s ", get_ls_gname(files[i].st.st_gid)); // 打印组名
            print_ls_size_hr(files[i].st.st_size, opts.human_readable); // 打印文件大小
            printf("%s", get_ls_mtime(files[i].st.st_mtime, tbuf, sizeof(tbuf))); // 打印修改时间
            printf(" %s\n", files[i].name);          // 打印文件名换行
        }                                            // 条件结束
        else                                         // 没有-l选项
        {
            printf("%s  ", files[i].name);           // 只打印文件名
        }                                            // 条件结束
    }                                                // 循环结束
    
    if(!opts.long_format && file_count > 0)          // 如果没有-l选项且有文件
    {
        printf("\n");                                // 打印换行
    }                                                // 条件结束
